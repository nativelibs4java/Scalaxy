...
* Version 0.3.2: More bugfixes for `flatMap`
* Version 0.3.1: Bugfixes for tuples (`i => (i, i)`), for `flatMap`...
* Version 0.3.0: Safe rewrites by default, configurable optimization strategies, more sources / ops (Option, flatMap...)
* Version 0.2.x: Unsafe rewrites with tuple rewiring.

# TODO

* Null tests for tuple unapply withFilter calls
* Remove `ScalarOp` limitation on reduction-like ops (`exists`, `min`, `getOrElse`...) to unlock full potential of `flatMap`
* Make sure scala-library builds with the plugin (safe mode), again.
* Fix `a pure expression does nothing in statement position` warnings (due to `""` block guards in AOT fake-typing blocks?)
